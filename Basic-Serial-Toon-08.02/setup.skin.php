...
<div class="tbl_head01 tbl_wrap">
	...
	<tr>
		...
		<td>
			<?php echo help('표지 등록 가능한 회원아이디를 콤마(,)구분해서 띄어쓰기 없이 등록');?>
			<input type="text" name="boset[smb_list]" id="smb_list" value="<?php echo ($boset['smb_list']);?>" size="60" class="frm_input"> 
		</td>
	</tr>
	<tr>
		<td align="center">연재등록</td>
		<td>
			<label><input type="checkbox" name="boset[swrite]" value="1"<?php echo get_checked('1', $boset['swrite']);?>> 표지글 등록회원 뿐만 아니라 누구나 연재글 등록 가능</label>
		</td>
	</tr>
	<tr>
		<td align="center">연재검색</td>
		<td>
			<label><input type="checkbox" name="boset[sstx]" value="1"<?php echo get_checked('1', $boset['sstx']);?>> 검색시 글목록에 연재글도 출력</label>
		</td>
	</tr>
	<tr>
		<td align="center">연재목록</td>
		<td>
			<?php echo help('글내용에서 페이지당 연재글 목록수로 기본값은 10 입니다.');?>
			PC 목록수 <input type="text" name="boset[spage_rows]" value="<?php echo $boset['spage_rows'];?>" size="4" class="frm_input" placeholder="10"> 개
			&nbsp;
			모바일 목록수 <input type="text" name="boset[smpage_rows]" value="<?php echo $boset['smpage_rows'];?>" size="4" class="frm_input" placeholder="10"> 개
		</td>
	</tr>
	</tbody>
	</table>
	...
	<p>
		<b>■ 연재스킨</b>
		&nbsp;
		<select name="boset[serial_skin]" onchange="apms_change_skin('serial_skin', 'serial', this.value);">
		<?php
			unset($skinlist);
			$skinlist = get_skin_dir('serial', $board_skin_path);
			$boset['serial_skin'] = (is_dir($board_skin_path.'/serial/'.$boset['serial_skin'])) ? $boset['serial_skin'] : $skinlist[0];
			for ($k=0; $k<count($skinlist); $k++) {
				echo "<option value=\"".$skinlist[$k]."\"".get_selected($skinlist[$k], $boset['serial_skin']).">".$skinlist[$k]."</option>\n";
			} 
		?>
		</select>
	</p>
	<div id="serial_skin">
		<?php @include_once($board_skin_path.'/serial/'.$boset['serial_skin'].'/setup.skin.php');?>
	</div>
