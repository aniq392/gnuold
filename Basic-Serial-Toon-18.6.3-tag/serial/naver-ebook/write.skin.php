...
<input type="hidden" name="sup" value="<?php echo $write['as_update']; ?>">
<?php
	$option = '';
	$option_hidden = '';
...
<?php if ($is_category || $option) { ?>
	<div class="form-group">
	<?php if($is_category && $serial_subject) { //표지 분류 hidden 으로 등록 ?>
	<input type="hidden" name="ca_name" value="<?php echo $swr['ca_name'];?>">
	<?php } else { ?>
...
<?php if($serial_subject) { //표지 제목 ?>
	<div class="form-group">
		<label class="col-sm-2 control-label">표지</label>
		<div class="col-sm-10">
			<label class="control-label sp-label">
				<b><?php echo $serial_subject;?></b>
			</label>
		</div>
	</div>
<?php } ?>
