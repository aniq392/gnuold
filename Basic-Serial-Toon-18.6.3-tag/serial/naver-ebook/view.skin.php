...
<!-- tail에 #1 소스 추가함 -->
<script>
 /*수정후 리모컨위젯 하단에 스크립트 추가되어야함*/
function remote_onoff() {
        $(".remote").toggle();
        if ($("t").is(".fa-toggle-on")) {
			...
			<div class="panel-heading">
				<div class="ellipsis text-muted<?php echo $view_font;?>">
					...
					<span class="pull-right">
						...
						<!-- 리모컨 온오프-->
						<span class="hidden-xs" style="color:#1dc800; cursor:pointer;" onclick="remote_onoff()" data-original-title="리모컨" data-toggle="tooltip"> | <i class="fa fa-plus-circle fa-lg" aria-hidden="true" style="color:#1dc800;"></i> 리모컨 <t name="remotog" class="fa fa-toggle-on"></t></span>
...
<link href="<?php echo $serial_skin_url ?>/booklet/jquery.booklet.latest.css" type="text/css" rel="stylesheet" media="screen, projection, tv" />
...
<script src="<?php echo $serial_skin_url ?>/booklet/jquery.easing.1.3.js"></script>
<script src="<?php echo $serial_skin_url ?>/booklet/jquery.booklet.latest.min.js"></script>
...
$(function() {
    $('#mybook').booklet({
        width:  <?php echo $boset['w_wr_8_text']; ?>,
        height: <?php echo $boset['w_wr_9_text']; ?>
	});
});
</script>
